...
def main(_):
    ...
    if runLCRROTALT == True:
        ...
        # k-fold cross validation
        for i in range(split_size):
            acc1, _, _, _, _, _ = lcrModelAlt_hierarchical_v4.main(BASE_train + str(i) + '.txt', test + str(i) + '.txt', accuracyOnt,
                                                   test_size[i], remaining_size, augment_data,
                                                   FLAGS.augmentation_file_path, ct={})
            acc.append(acc1)
...
def main(_):
    ...
    BASE_train = "C:/Users/Roel/PycharmProjects/venv36/data/programGeneratedData/crossValidation"+str(FLAGS.year)+'/cross_train_'
    BASE_val = "C:/Users/Roel/PycharmProjects/venv36/data/programGeneratedData/crossValidation"+str(FLAGS.year)+'/cross_val_'
    BASE_svm_train = "C:/Users/Roel/PycharmProjects/venv36/data/programGeneratedData/crossValidation"+str(FLAGS.year)+'/svm/cross_train_svm_'
    BASE_svm_val = "C:/Users/Roel/PycharmProjects/venv36/data/programGeneratedData/crossValidation"+str(FLAGS.year)+'/svm/cross_val_svm_'


    REMAIN_val = "C:/Users/Roel/PycharmProjects/venv36/data/programGeneratedData/crossValidation"+str(FLAGS.year)+'/cross_val_remainder_'
    REMAIN_svm_val = "C:/Users/Roel/PycharmProjects/venv36/programGeneratedData/crossValidation"+str(FLAGS.year)+'/svm/cross_val_remainder_'
    ...
    if useOntology == True:
        ...
        for i in range(split_size):
            ...
            acc.append(accuracyOnt)
            remaining_size_vec.append(remaining_size)
        with open("C:/Users/Roel/PycharmProjects/venv36/Desktop/data/programGeneratedData/crossValidation"+str(FLAGS.year)+"/cross_results_"+str(FLAGS.year)+"/ONTOLOGY_"+str(FLAGS.year)+".txt", 'w') as result:
            print(str(split_size)+'-fold cross validation results')
            print('Accuracy: {}, St Dev:{}'.format(np.mean(np.asarray(acc)), np.std(np.asarray(acc))))
            print(acc)
            result.write('size:' + str(test_size))
    ...
    if runLCRROTALT == True:
        ...
        #k-fold cross validation
        for i in range(split_size):
            acc1, _, _, _, _, _ = lcrModelAlt_hierarchical_v4.main(BASE_train+str(i)+'.txt',test+str(i)+'.txt', accuracyOnt, test_size[i], remaining_size, augment_data,
                                                                       FLAGS.augmentation_file_path,
                                                                       ct={})
            acc.append(acc1)
            tf.reset_default_graph()
            print('iteration: '+ str(i))
        with open("C:/Users/Roel/PycharmProjects/venv36/data/programGeneratedData/crossValidation"+str(FLAGS.year)+"/cross_results_"+str(FLAGS.year)+"/LCRROT_ALT_"+str(FLAGS.year)+".txt", 'w') as result:
            result.write(str(acc))
            result.write('Accuracy: {}, St Dev:{} /n'.format(np.mean(np.asarray(acc)), np.std(np.asarray(acc))))
            print(str(split_size)+'-fold cross validation results')
            print('Accuracy: {}, St Dev:{}'.format(np.mean(np.asarray(acc)), np.std(np.asarray(acc))))
