...
lcrspace = [
    hp.choice('learning_rate', [0.02, 0.05, 0.07,  0.09]),
    hp.quniform('keep_prob', 0.25, 0.75, 0.1),
    hp.choice('momentum', [0.85, 0.9, 0.95, 0.99]),
    hp.choice('l2', [0.00001, 0.0001, 0.001, 0.01]),
]
...
def lcr_alt_objective(hyperparams):
    ...
    l, pred1, fw1, bw1, tl1, tr1 = lcrModelAlt_hierarchical_v4.main(FLAGS.hyper_train_path,
                                                                    FLAGS.hyper_eval_path,
                                                                    accuracyOnt,
                                                                    test_size,
                                                                    remaining_size,
                                                                    augment_data,
...
def svm_objective(hyperparams):
    ...
    l = svmModel.main(FLAGS.hyper_svm_train_path, FLAGS.hyper_svm_eval_path, accuracyOnt, test_size, remaining_size, c,
                      gamma)
    tf.reset_default_graph()
...
def load_best_hyperspace():
    ...
    return load_json_result(best_result_name)
