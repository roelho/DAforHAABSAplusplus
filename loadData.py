...
def loadDataAndEmbeddings(config,loadData, augment_data):
    ...
    if loadData == True:
        ...
        return len(train_data[0]), len(test_data[0]), train_data[4], test_data[4], ct  # train_size, test_size, train_polarity_vector, test_polarity_vector
...
def loadCrossValidation(config, split_size, augment_data, load=True):
    FLAGS = config
    if load:
        words, sent, words_t, sent_t = [], [], [], []

        with open(FLAGS.train_path, encoding='cp1252') as f, open(FLAGS.test_path, encoding='cp1252') as test:
            lines = f.readlines()
            ...
            for train_idx, val_idx in kf.split(words_t, sent_t):
                ...
                # svmwords_1 = svmwords[train_idx]
                # svmwords_2 = svmwords[val_idx]
                with open("data/programGeneratedData/crossValidation" + str(FLAGS.year) + '/cross_val_' + str(
                            i) + '.txt', 'w') as val:
                    for row in words_1:
                        val.write(row[0])
                        val.write(row[1])
