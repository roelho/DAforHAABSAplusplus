...
def main(train_path, test_path, accuracyOnt, test_size, remaining_size, learning_rate=FLAGS.learning_rate, keep_prob=FLAGS.keep_prob1, momentum=0.95, l2=0.00001):
    print_config()
    ...
    with tf.Session(config=config) as sess:
        ...
        te_x, te_sen_len, te_x_bw, te_sen_len_bw, te_y, te_target_word, te_tar_len, te_all_sent, te_all_target, all_y = load_inputs_twitter(
            test_path,
            word_id_mapping,
            FLAGS.max_sentence_len,
            'TC',
        ...
        for i in range(FLAGS.n_iter):
            ...
            for test, num in get_batch_data(te_x, te_sen_len, te_x_bw, te_sen_len_bw, te_y,
                                            te_target_word, te_tar_len, 1000, 1.0, 1.0, False):
        ...
        return max_acc, np.where(np.subtract(max_py, max_ty) == 0, 0, 1), max_fw.tolist(), max_bw.tolist(), max_tl.tolist(), max_tr.tolist(), te_all_sent.tolist(), te_all_target.tolist(), all_y.tolist()
