...
''''''
temp_filenames_base = ['data/temporaryData/temp_BERT/BERT_test0till650.txt',
                       'data/temporaryData/temp_BERT/BERT_base0till1000.txt',
                       'data/temporaryData/temp_BERT/BERT_base1000till2000.txt',
                       'data/temporaryData/temp_BERT/BERT_base2001till3300.txt', 'data/temporaryData/temp_BERT/BERT_base3301till4100.txt',
                       'data/temporaryData/temp_BERT/BERT_base4101till5000.txt'
                       ]
'''
temp_filenames_base = ['data/temporaryData/temp_BERT/BERT_remaining/eda/embedding_remain+sr+ri+rs.txt']


count_sentences=0
with open('BERT_base_2016embedding.txt','w') as outf:
...
with open('BERT_base_2016embedding_withCLS_SEP.txt','w') as outf:
    for tfname in temp_filenames_base:
        print(tfname)
        with open(tfname) as infile:
            for line in infile:
...
unique_words = []
unique_words_index = []
with open('BERT_base_2016embedding_withCLS_SEP.txt') as BERTemb_sep:
...
counter = 0
uniqueVocaBERT=[]
with open('BERT_base_2016embedding.txt') as BERTemb:
...
lines = open('data/externalData/berttijdelijk.txt').readlines()
index = 0
index_sep = 0
x_word = []
x_sent = []
...
with open('data/programGeneratedData/'+str(FLAGS.embedding_dim) +'testdata_EDA'+str(FLAGS.year) +'BERT.txt','w') as outTrain, \
        open('data/programGeneratedData/'+str(FLAGS.embedding_dim) +'traindata_EDA'+str(FLAGS.year) +'BERT.txt','w') as outTest:
    for j in range(0, 744):
        outTrain.write(linesAllData[j])
    for k in range(744,len(linesAllData)):
