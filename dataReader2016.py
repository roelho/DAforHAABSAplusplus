        ...
        for category in category_sorter.keys():
            ...
            for _ in range(FLAGS.EDA_swap):
                for i, j in tqdm(zip(*[iter(indices)] * 2)):
                    adder = 0
                    while sentences_same_cat[i].get('aspect') == sentences_same_cat[
                        (j + adder) % len(indices)].get(
                        'aspect') and adder < 100:  # happens more than you think
                        adder += 1
                    sent1, sent2 = augmenter.swap_targets(sentences_same_cat[i], sentences_same_cat[
                        (j + adder) % len(indices)])
